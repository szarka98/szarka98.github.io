...
for (var i = 0; i < 5; i++) {
    ...
    v0_[i] = [leftX + size * i, 100];
    v[i] = [v0[i][0], v0[i][1]];
    v_[i] = [v0_[i][0], v0_[i][1]];
...
function draw() {
    ...
    for (var i = 0; i < 5; i++) {
        drawElem(i);
        if (!magnet) {
            if (v[i][0] + dv[i][0] * speed > ww - size || v[i][0] + dv[i][0] * speed < 0) {
                speed = 3;
                dv[i][0] = -dv[i][0];
            }
            if (v[i][1] + dv[i][1] * speed > wh - size || v[i][1] + dv[i][1] * speed < 0) {
                speed = 3;
                dv[i][1] = -dv[i][1];
            }
            if (v_[i][0] + dv_[i][0] * speed > ww - size || v_[i][0] + dv_[i][0] * speed < 0) {
                speed = 3;
                dv_[i][0] = -dv_[i][0];
            }
            if (v_[i][1] + dv_[i][1] * speed > wh - size || v_[i][1] + dv_[i][1] * speed < 0) {
                speed = 3;
                dv_[i][1] = -dv_[i][1];
            }
            v[i][0] += dv[i][0] * speed;
            ...
            speed = 6;
            if (Math.abs(v0[i][0] - v[i][0]) > 4 || Math.abs(v0[i][1] - v[i][1]) > 4) {
                v[i][0] = v[i][0] + speed * (v0[i][0] - v[i][0]) / (Math.abs(v0[i][0] - v[i][0]) + Math.abs(v0[i][1] - v[i][1]));
                v[i][1] = v[i][1] + speed * (v0[i][1] - v[i][1]) / (Math.abs(v0[i][0] - v[i][0]) + Math.abs(v0[i][1] - v[i][1]));
            } else {
                v[i][0] = v0[i][0];
                v[i][1] = v0[i][1];
            }
            if (Math.abs(v0_[i][0] - v_[i][0]) > 4 || Math.abs(v0_[i][1] - v_[i][1]) > 4) {
                v_[i][0] = v_[i][0] + speed * (v0_[i][0] - v_[i][0]) / (Math.abs(v0_[i][0] - v_[i][0]) + Math.abs(v0_[i][1] - v_[i][1]));
                v_[i][1] = v_[i][1] + speed * (v0_[i][1] - v_[i][1]) / (Math.abs(v0_[i][0] - v_[i][0]) + Math.abs(v0_[i][1] - v_[i][1]));
            } else {
                v_[i][0] = v0_[i][0];
                v_[i][1] = v0_[i][1];
            }
