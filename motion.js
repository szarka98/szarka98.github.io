...
var dv = [];
for (var i = 0; i < 5; i++) {
  v0[i] = [[leftX + size * i, 100], [leftX + size * i, 200], [leftX + size * i, 300]];
  v[i] = [[v0[i][0][0], v0[i][0][1]], [v0[i][1][0], v0[i][1][1]], [v0[i][2][0], v0[i][2][1]]];
  dv[i] = [
...
var speed = 12;
var elemName = [["St", "At", "E", " ", "Of"], ["Ja", "Va", "Sc", "Ri", "Pt"], [" ", " ", "20", "18", " "]];
...
function drawElem(i) {
  if (elemBoole[2][i]) {
    ctx.beginPath();
    // box
    ctx.rect(v[i][2][0], v[i][2][1], size, size);
    ctx.fillStyle = "#212424";
    ctx.fillRect(v[i][2][0], v[i][2][1], size, size);
    ...
    ctx.fillStyle = elemNameColor[2];
    ctx.fillText(elemName[2][i], v[i][2][0] + size / 2, v[i][2][1] + size * 0.6);
    // number
...
function draw() {
  ...
  for (var i = 0; i < 5; i++) {
    ...
    if (!magnet) {
      ...
      if (
        v[i][2][0] + dv[i][2][0] * speed > ww - size ||
        v[i][2][0] + dv[i][2][0] * speed < 0
      ) {
        speed = 3;
        dv[i][2][0] = -dv[i][2][0];
      }
      if (
        v[i][2][1] + dv[i][2][1] * speed > wh - size ||
        v[i][2][1] + dv[i][2][1] * speed < 0
      ) {
        speed = 3;
        dv[i][2][1] = -dv[i][2][1];
      }
      ...
      v[i][2][0] += dv[i][2][0] * speed;
      v[i][2][1] += dv[i][2][1] * speed;
      ...
      if (
        Math.abs(v0[i][2][0] - v[i][2][0]) > speed ||
        Math.abs(v0[i][2][1] - v[i][2][1]) > speed
      ) {
        v[i][2][0] =
          v[i][2][0] +
          (speed * (v0[i][2][0] - v[i][2][0])) /
            (Math.abs(v0[i][2][0] - v[i][2][0]) +
              Math.abs(v0[i][2][1] - v[i][2][1]));
        v[i][2][1] =
          v[i][2][1] +
          (speed * (v0[i][2][1] - v[i][2][1])) /
            (Math.abs(v0[i][2][0] - v[i][2][0]) +
              Math.abs(v0[i][2][1] - v[i][2][1]));
      } else {
        v[i][2][0] = v0[i][2][0];
      ...
      if (
        Math.abs(v0[i][1][0] - v[i][1][0]) > speed ||
        Math.abs(v0[i][1][1] - v[i][1][1]) > speed
      ) {
        v[i][1][0] =
          v[i][1][0] +
      ...
      if (
        Math.abs(v0[i][0][0] - v[i][0][0]) > speed ||
        Math.abs(v0[i][0][1] - v[i][0][1]) > speed
      ) {
        v[i][0][0] =
          v[i][0][0] +
